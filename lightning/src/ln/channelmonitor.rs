...
impl<ChanSigner: ChannelKeys> ChannelMonitor<ChanSigner> {
	...
	fn check_spend_remote_transaction<L: Deref>(&mut self, tx: &Transaction, height: u32, logger: &L) -> (Vec<OnchainRequest>, (Txid, Vec<(u32, TxOut)>)) where L::Target: Logger {
	...
	fn check_spend_remote_htlc<L: Deref>(&mut self, tx: &Transaction, commitment_number: u64, height: u32, logger: &L) -> (Vec<OnchainRequest>, Option<(Txid, Vec<(u32, TxOut)>)>) where L::Target: Logger {
	...
	fn broadcast_by_local_state(&self, commitment_tx: &Transaction, local_tx: &LocalSignedTx, height: u32) -> (Vec<OnchainRequest>, Vec<(u32, TxOut)>, Option<(Script, PublicKey, PublicKey)>) {
	...
	fn check_spend_local_transaction<L: Deref>(&mut self, tx: &Transaction, height: u32, logger: &L) -> (Vec<OnchainRequest>, (Txid, Vec<(u32, TxOut)>)) where L::Target: Logger {
