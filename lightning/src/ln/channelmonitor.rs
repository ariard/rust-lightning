			...
			for inp in &tx.input {
				if let Some(ancestor_claimable_txid) = self.claimable_outpoints.get(&inp.previous_output) {
					...
					if let Some(claim_material) = self.pending_claim_requests.get_mut(&ancestor_claimable_txid.0) {
						...
						// before we could anyway with same inputs order than us), wait for ANTI_REORG_DELAY and clean the RBF
						// tracking map.
						if set_equality {
							...
							for input in tx.input.iter() {
								...
								if claim_material.per_input_material.is_empty() { // If no outpoints left to claim in this request clean it after ANTI_REORG_DELAY
									clean_claim_request_after_safety_delay!();
								}
							}
