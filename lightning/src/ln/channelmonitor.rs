...
use bitcoin::blockdata::transaction::{TxOut,SigHashType,Transaction};
...
impl ChannelMonitor {
	pub(super) fn new(funding_key: &SecretKey, revocation_base_key: &SecretKey, delayed_payment_base_key: &SecretKey, htlc_base_key: &SecretKey, payment_base_key: &SecretKey, shutdown_pubkey: &PublicKey, our_to_self_delay: u16, destination_script: Script, logger: Arc<Logger>) -> ChannelMonitor {
		ChannelMonitor {
			...
			onchain_tx_handler: OnchainTxHandler::new(destination_script, logger.clone()),
	...
	fn check_spend_remote_transaction(&mut self, tx: &Transaction, height: u32) -> (HashMap<Sha256dHash, Vec<(u32, bool, BitcoinOutPoint, InputMaterial)>>, (Sha256dHash, Vec<TxOut>), Option<SpendableOutputDescriptor>) {
		...
		let mut spendable_descriptor = None;
		...
		macro_rules! ignore_error {
			( $thing : expr ) => {
				match $thing {
					Ok(a) => a,
					Err(_) => return (claimable_outpoints, (commitment_txid, watch_outputs), spendable_descriptor)
				}
			};
		}
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			let a_htlc_key = match self.their_htlc_base_key {
				None => return (claimable_outpoints, (commitment_txid, watch_outputs), spendable_descriptor),
		...
		if outpoints.len() > 0 { claimable_outpoints.insert(commitment_txid, outpoints); }
		(claimable_outpoints, (commitment_txid, watch_outputs), spendable_descriptor)
	}
