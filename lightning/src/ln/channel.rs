...
impl<ChanSigner: ChannelKeys> Channel<ChanSigner> {
	...
	fn build_commitment_transaction<L: Deref>(&self, commitment_number: u64, keys: &TxCreationKeys, building_for_counterparty: bool, feerate_per_kw: u64, logger: &L) -> (Transaction, usize, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>) where L::Target: Logger {
		...
		//XXX: delete local/generated_by_local distinction
		//XXX rename variable names, drop remote mention
		...
		#[cfg(debug_assertions)]
		{
			...
			debug_assert!(max_commitment_tx_output.0 <= value_to_self_msat as u64 || value_to_self_msat / 1000 >= self.channel_reserve_satoshis as i64);
		...
		let (value_to_a, value_to_b) = if !building_for_counterparty { (value_to_self, value_to_remote) } else { (value_to_remote, value_to_self) };
