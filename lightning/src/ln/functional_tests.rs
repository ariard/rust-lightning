...
fn test_justice_tx() {
	...
	assert_eq!(revoked_local_txn[0].output.len(), 3); // Only HTLC and output back to 0 are present (+ 0's anchor)
	...
	assert_eq!(revoked_local_txn[0].output.len(), 3); // Only HTLC and output back to A are present (+ A's anchor)
	check_anchor_output(&revoked_local_txn[0], 1);
	// Revoke the old state
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
	...
	// Get the will-be-revoked local txn from nodes[2]
	let revoked_local_txn = get_local_commitment_txn!(nodes[2], chan_2.2);
	assert_eq!(revoked_local_txn[0].output.len(), if no_to_remote { 2 } else { 3 });
	check_anchor_output(&revoked_local_txn[0], 1);
	// Revoke the old state
...
fn test_claim_sizeable_push_msat() {
	...
	assert_eq!(node_txn[0].output.len(), 4); // We can't force trimming of to_remote output as channel_reserve_satoshis block us to do so at channel opening
