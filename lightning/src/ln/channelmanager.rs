        ...
        L::Target: Logger,
{
		...
		if let &PendingHTLCStatus::Forward(PendingHTLCInfo { ref routing, ref amt_to_forward, ref outgoing_cltv_value, .. }) = &pending_forward_info {
			...
			if let &PendingHTLCRouting::Forward { ref short_channel_id, .. } = routing {
				...
				if let Some((err, code, chan_update)) = loop {
					...
					let fee = amt_to_forward.checked_mul(chan.get_fee_proportional_millionths() as u64).and_then(|prop_fee| { (prop_fee / 1000000).checked_add(chan.get_fee_base_msat(&self.fee_estimator) as u64) });
	...
	fn get_channel_update(&self, chan: &Channel<ChanSigner>) -> Result<msgs::ChannelUpdate, LightningError> {
		...
		let unsigned = msgs::UnsignedChannelUpdate {
			...
			htlc_minimum_msat: chan.get_htlc_minimum_msat(),
			fee_base_msat: chan.get_fee_base_msat(&self.fee_estimator),
	...
	fn read<R: ::std::io::Read>(reader: &mut R, args: ChannelManagerReadArgs<'a, ChanSigner, M, T, K, F, L>) -> Result<Self, DecodeError> {
		...
		for _ in 0..channel_count {
			...
			if let Some(ref mut monitor) = args.channel_monitors.get_mut(&funding_txo) {
				if channel.get_cur_commitment_transaction_number() < monitor.get_cur_local_commitment_number() ||
					...
					return Err(DecodeError::InvalidValue);
				} else if channel.get_cur_commitment_transaction_number() > monitor.get_cur_local_commitment_number() ||
						channel.get_revoked_remote_commitment_transaction_number() > monitor.get_min_seen_secret() ||
						channel.get_cur_counterparty_commitment_transaction_number() > monitor.get_cur_remote_commitment_number() ||
						channel.get_latest_monitor_update_id() < monitor.get_latest_update_id() {
