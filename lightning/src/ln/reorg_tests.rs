...
fn do_test_onchain_htlc_reorg(local_commitment: bool, claim: bool) {
	...
	let claim_txn = if local_commitment {
		...
		assert_eq!(node_1_commitment_txn[0].output.len(), 3); // to-self + Offered HTLC (to-remote/to-node-3 is dust) + anchor output
		check_anchor_output(&node_1_commitment_txn[0], 1);
		...
		assert_eq!(node_2_commitment_txn[1].output.len(), 3); // to-remote + Received HTLC (to-self is dust) + anchor output
		check_anchor_output(&node_2_commitment_txn[1], 1);
		...
		assert_eq!(node_2_commitment_txn[0].output.len(), 3); // to-remote + Received HTLC (to-self is dust) + anchor output
		check_anchor_output(&node_2_commitment_txn[0], 1);
		...
		assert_eq!(node_1_commitment_txn[1].output.len(), 3); // to-local and Offered HTLC (to-remote is dust) + anchor output
		check_anchor_output(&node_1_commitment_txn[1], 1);
