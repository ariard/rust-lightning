...
impl ChannelMonitor {
	...
	pub fn insert_combine(&mut self, mut other: ChannelMonitor) -> Result<(), MonitorUpdateError> {
		match self.key_storage {
			Storage::Local { ref funding_info, ref mut short_channel_id, .. } => {
				...
				if let Storage::Local { ref funding_info, ref short_channel_id, .. } = other.key_storage {
					if funding_info.is_none() { return Err(MonitorUpdateError("Try to combine a Local monitor without funding_info")); }
					...
					if funding_info.as_ref().unwrap().0 != our_funding_info.as_ref().unwrap().0 {
						...
						if short_channel_id.is_some() && our_short_channel_id.is_some() {
							if short_channel_id != our_short_channel_id {
								return Err(MonitorUpdateError("Tried to combine monitors with different short ids!"));
							}
						} else if our_short_channel_id.is_none() {
							*our_short_channel_id = *short_channel_id;
						}
					}
				} else {
