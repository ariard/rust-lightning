...
pub struct ChannelMonitor {
	...
	// Used to track outpoint in the process of being claimed by our transactions. If height timer (u32) is expired
	// and claim tx hasn't reached enough confirmations before, use TxMaterial to regenerate a new claim tx with
	// a satoshis-per-1000-weight-units higher than last one (u64).
...
impl ChannelMonitor {
					...
					for (idx, &(ref htlc, _)) in per_commitment_data.iter().enumerate() {
						if let Some(transaction_output_index) = htlc.transaction_output_index {
							...
							if !htlc.offered {
								...
								let (_redeemscript, _htlc_key) = sign_input!(sighash_parts, timeout_tx.input[0], htlc.amount_msat / 1000, vec![0]);
								//TODO: we should delay broadcast of timeout tx until locktime - 1 to avoid being bounce off by mempool
	...
	pub(super) fn get_latest_local_commitment_txn(&self) -> Vec<Transaction> {
		if let &Some(ref local_tx) = &self.current_local_signed_commitment_tx {
			...
			match self.key_storage {
				Storage::Local { ref delayed_payment_base_key, ref prev_latest_per_commitment_point, .. } => {
					res.append(&mut self.broadcast_by_local_state(local_tx, prev_latest_per_commitment_point, &Some(*delayed_payment_base_key), 0).0); // Accurate height doesn't matter as we're going to change 
				},
