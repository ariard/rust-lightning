...
impl ChannelManager {
	...
	fn get_channel_update(&self, chan: &Channel) -> Result<msgs::ChannelUpdate, ErrorPacket> {
		let short_channel_id = match chan.get_short_channel_id() {
			None => return Err(ErrorPacket{err: "Channel not yet established", action: msgs::ErrorAction::IgnoreError}),
			Some(id) => id,
		};
	...
	fn internal_announcement_signatures(&self, their_node_id: &PublicKey, msg: &msgs::AnnouncementSignatures) -> Result<(), MsgHandleErrInternal> {
		...
		match channel_state.by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan) => {
				...
				if !chan.get().is_usable() {
					return Err(MsgHandleErrInternal::from_no_close(ErrorPacket{err: "Got an announcement_signatures before we were ready for it", action: msgs::ErrorAction::IgnoreError}));
				}
