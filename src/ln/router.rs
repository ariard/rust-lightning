...
impl RoutingMessageHandler for Router {
	fn handle_node_announcement(&self, msg: &msgs::NodeAnnouncement) -> Result<bool, ErrorPacket> {
	...
	fn handle_channel_update(&self, msg: &msgs::ChannelUpdate) -> Result<bool, ErrorPacket> {
		let mut network = self.network_map.write().unwrap();
		let dest_node_id;
...
impl Router {
	...
	pub fn get_route(&self, target: &PublicKey, first_hops: Option<&[channelmanager::ChannelDetails]>, last_hops: &[RouteHint], final_value_msat: u64, final_cltv: u32) -> Result<Route, ErrorPacket> {
