...
fn test_insane_channel_opens() {
	...
	let insane_open_helper = |expected_error_str, message_mutator: fn(msgs::OpenChannel) -> msgs::OpenChannel| {
		match nodes[1].node.handle_open_channel(&nodes[0].node.get_our_node_id(), LocalFeatures::new(), &message_mutator(open_channel_message.clone())) {
			Err(msgs::ErrorPacket{ err: error_str, action: msgs::ErrorAction::SendErrorMessage {..}}) => {
...
fn do_test_shutdown_rebroadcast(recv_count: u8) {
	...
	if recv_count == 0 {
		...
		if let Err(msgs::ErrorPacket{action: msgs::ErrorAction::SendErrorMessage{msg}, ..}) =
...
fn test_manager_serialize_deserialize_inconsistent_monitor() {
	...
	if let Err(msgs::ErrorPacket { action: msgs::ErrorAction::SendErrorMessage { msg }, .. }) = nodes[0].node.handle_channel_reestablish(&nodes[3].node.get_our_node_id(), &reestablish) {
...
fn test_update_add_htlc_bolt2_receiver_check_amount_received_more_than_min() {
	...
	if let Err(msgs::ErrorPacket{err, action: msgs::ErrorAction::SendErrorMessage {..}}) = err {
