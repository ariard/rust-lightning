...
impl PeerChannelEncryptor {
	...
	#[inline]
	fn decrypt_with_ad(res: &mut[u8], n: u64, key: &[u8; 32], h: &[u8], cyphertext: &[u8]) -> Result<(), ErrorPacket> {
		let mut nonce = [0; 12];
		nonce[4..].copy_from_slice(&byte_utils::le64_to_array(n));

		let mut chacha = ChaCha20Poly1305RFC::new(key, &nonce, h);
		if !chacha.decrypt(&cyphertext[0..cyphertext.len() - 16], res, &cyphertext[cyphertext.len() - 16..]) {
			return Err(ErrorPacket{err: "Bad MAC", action: Some(msgs::ErrorAction::DisconnectPeer{ msg: None })});
		}
		Ok(())
	}
	...
	#[inline]
	fn inbound_noise_act(state: &mut BidirectionalNoiseState, act: &[u8], our_key: &SecretKey) -> Result<(PublicKey, [u8; 32]), ErrorPacket> {
		assert_eq!(act.len(), 50);

		if act[0] != 0 {
			return Err(ErrorPacket{err: "Unknown handshake version number", action: Some(msgs::ErrorAction::DisconnectPeer{ msg: None })});
		}

		let their_pub = match PublicKey::from_slice(&act[1..34]) {
			Err(_) => return Err(ErrorPacket{err: "Invalid public key", action: Some(msgs::ErrorAction::DisconnectPeer{ msg: None })}),
			Ok(key) => key,
		};
	...
	pub fn process_act_one_with_keys(&mut self, act_one: &[u8], our_node_secret: &SecretKey, our_ephemeral: SecretKey) -> Result<[u8; 50], ErrorPacket> {
	...
	pub fn process_act_two(&mut self, act_two: &[u8], our_node_secret: &SecretKey) -> Result<([u8; 66], PublicKey), ErrorPacket> {
	...
	pub fn process_act_three(&mut self, act_three: &[u8]) -> Result<PublicKey, ErrorPacket> {
				...
				match directional_state {
					&DirectionalNoiseState::Inbound { ie: _, ref re, ref temp_k2 } => {
						...
						self.their_node_id = Some(match PublicKey::from_slice(&their_node_id) {
							Ok(key) => key,
							Err(_) => return Err(ErrorPacket{err: "Bad node_id from peer", action: Some(msgs::ErrorAction::DisconnectPeer{ msg: None })}),
						});
	...
	pub fn decrypt_length_header(&mut self, msg: &[u8]) -> Result<u16, ErrorPacket> {
	...
	pub fn decrypt_message(&mut self, msg: &[u8]) -> Result<Vec<u8>, ErrorPacket> {
		if msg.len() > 65535 + 16 {
			panic!("Attempted to encrypt message longer than 65535 bytes!");
		}
